...
t.test('handles unknown error with logs and debug file', async (t) => {
  ...
  const debugContentLogs = debugLines.map(l => l.replace(/^\d+\s/, 'XX '))
  const lastLog = debugLines.reduce((__, l) => parseInt(l.match(/^(\d+)\s/)[1]))
  ...
  t.matchSnapshot(debugContentLogs, 'debug file contents')
})
